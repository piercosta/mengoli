...
executeInput(G):- 		%%Perhaps a built-in rule
	actorPrintln( executeInput11111111(G) ), 
	(G,!; do( [ true ] , G)),
	setAnswer( done(G) ),
	actorobj(Actor),
	Actor <- removeTout returns N,
	actorPrintln(  removerunDoSentence(N)  ).
executeInput(X):- 	
	actorPrintln( executeInput222222(X)  ),
	doInput(X).
...
do( [ GUARD ], MOVE, DURATION , ENDEV ) :-	 
 	...
 	actorPrintln( dooooo( Actor, GUARD, MOVE, DURATION, ENDEV ) ),
	do( Actor, GUARD, MOVE, DURATION, ENDEV   ),!.
do( [ true ], G1, G2 ,G3 ) :-	 
 	executeAndProlog( [G1, G2, G3] ).
do( [ GUARD ], MOVE, DURATION , EVENTS, PLANS) :-	 
	actorobj(Actor), number(DURATION), !, DURATION >= 0,  
	%%actorPrintln( do( Actor, GUARD, MOVE, DURATION, EVENTS, PLANS ) ),
...
do( Actor, GUARD, move(CMD, SPEED, ANGLE), DURATION, ENDEV  ):- !,
	actorPrintln( aaaa(Actor) ),
	runDoSentence(Actor, sentence( GUARD, move( robotmove, CMD, SPEED, DURATION, ANGLE), ENDEV) ). %%  
...
runDoSentence(Actor, SENTENCE ) :-
	actorPrintln(  runDoSentence( Actor, SENTENCE   ) ), 
  	curPlan(PLANNAME),
 	inc( pc,1,PC ) ,
 	actorPrintln(  goingTorunTheSentence( Actor, SENTENCE  ) ), 
...
/*
do( Actor, GUARD, move(CMD, SPEED, ANGLE), DURATION, EVENTS, PLANS ):-!,
	runDoSentence(Actor, sentence( GUARD, move( robotmove, CMD, SPEED, DURATION, ANGLE ), EVENTS, PLANS ) ).
 
do( Actor, GUARD, raise( EVID, CONTENT ), DURATION,  EVENTS, PLANS ):- !,
...
initTalk  :-  
	actorobj(Actor),
	assign( pc,0 ),
	actorPrintln( initTalkactorobj(Actor) ),
	( Actor <- isSimpleActor returns R, R=true, !,
	  actorPrintln(" *** talkTheoryAvatar loaded FOR ACTORS ONLY  ***  ");
	  actorPrintln(" *** talkTheoryAvatar loaded FOR ROBOTS  *** ")
	).
